...
function getFormatDate(date){
    ...
    month = month >= 10 ? month : '0' + month;  //m
    var day = date.getDate();                   //d
    day = day >= 10 ? day : '0' + day;          //d
...
날씨.prototype.날씨정보조회 = function(aInput){
    ...
    try {
        ...
        this.post = 'keyword='  +    _지역명;
        if(!httpRequest.postWithUserAgent(this.userAgent, this.host + '/search/api/searchResult', this.post)) {
            // 오류처리
        ...
        this.log('지역검색결과::: ' + Result );


        var searchfrm1 = StrGrab(Result, '"info_txt">', '</div>');

        if(StrGrab(searchfrm1,  '<strong class="tit">', '</strong>') != '') {
        ...
        var searchfrm2 = StrGrab(Result, '"result_none">', '</div>');

        if(StrGrab(searchfrm2, '/strong>', '<') == '에 대한 검색결과가 없습니다.') {
            this.setError(I_IBX_RESULT_NOTPRESENT);
            this.iSASInOut.Output.ErrorMessage = "지역명이 없습니다."; 
        ...
        if(!_지역명||!현재날씨||!현재기온||!오늘날짜||!최저기온 || !최고기온){
        ...
        if(!날씨정보조회.오늘.지역명||!날씨정보조회.오늘.오늘날짜||!날씨정보조회.오늘.현재날씨 ||!날씨정보조회.오늘.현재기온|| !날씨정보조회.오늘.오전날씨 || !날씨정보조회.오늘.오후날씨 ||!날씨정보조회.오늘.최저기온 ||!날씨정보조회.오늘.최고기온 ||!날씨정보조회.오늘.미세먼지등급 ||!날씨정보조회.오늘.초미세먼지등급     ){

            
            this.log("날씨정보조회::: " + JSON.stringify(날씨정보조회));
            this.setError(E_IBX_RESULT_FAIL);
            return E_IBX_RESULT_FAIL;
        }
...
///////////내일////////
       ...
       if(!내일오전날씨|| !내일오후날씨  ){
        ...
        if(!날씨정보조회.내일.내일날짜||!날씨정보조회.내일.내일오전날씨||!날씨정보조회.내일.내일오후날씨 ){
