...
## 17. deepinit (딥 코드베이스 초기화)

> `/oh-my-claudecode:deepinit` — 코드베이스를 훑어 **계층형 `AGENTS.md` 문서를 자동 생성**하는 초기화 스킬. 에이전트가 프로젝트 구조를 미리 이해하도록 "지도"를 깔아준다.

### 개념 한 줄
**루트에 `CLAUDE.md` 하나만 만드는 기본 `/init`보다 깊게 — 디렉터리·서브시스템별로 `AGENTS.md`를 계층적으로 만들어 준다.**

### 기본 `/init` 과의 차이

| 구분 | 기본 `/init` | OMC `deepinit` |
|------|-------------|----------------|
| 산출물 | 루트 `CLAUDE.md` 1개 | 디렉터리별 `AGENTS.md` **계층 구조** |
| 관점 | 프로젝트 개요 | 서브시스템 단위로 나눠 문서화 |
| 적합 | 소규모·단일 | **큰/멀티모듈 코드베이스** |

- 폴더마다 그 폴더용 `AGENTS.md`가 있으니, 에이전트가 그 안에서 작업할 때 **필요한 국소 맥락만** 참고하면 된다 → 컨텍스트 낭비를 줄이는 방향(🔗 [2026-06-27 노트](/2026/06/27/claude-code-notes_2.html) 6번 컨텍스트 엔지니어링과 같은 맥락).
- 생성된 문서 묶음은 매니페스트(`deepinit_manifest`)로 관리된다.

> 한 줄 요약: **deepinit = `/init`의 심화판 — 코드베이스를 계층형 `AGENTS.md`로 문서화해, 큰 프로젝트에서도 에이전트가 폴더별 맥락을 바로 잡게 하는 초기화 스킬.**
