...
## 6. 실제로 만들어본 예시 — 구독 다이어트 종합발표

> slides-grab으로 직접 만든 "구독 다이어트" 앱의 **제품 발표 덱**(26장). Plan→Design→Edit→Export를 거쳐 PDF로 뽑았다.

![slides-grab으로 만든 구독 다이어트 종합발표 덱](/assets/slides-grab-example.png)
> 그림: slides-grab 결과 — "PRODUCT DECK · 2026" 표지와 "발표 개요(OVERVIEW)". 밝은 테마의 발표용 덱으로, 폰 목업·기술 태그(Next.js·Prisma·Vercel)까지 제품 소개답게 구성됐다.

- 산출물: `subscription-diet/decks/full-deck/구독다이어트_종합발표.pdf`
- 같은 프로젝트라도 slides-grab은 **"제품 발표(소개)"** 성격으로 나온다 — frontend-slides로 만든 사용자 메뉴얼과 결이 다르다(🔗 [frontend-slides 노트](/2026/07/11/frontend-slides.html)).

> 한 줄 요약: **slides-grab으로 구독 다이어트 제품 발표 덱(26장, PDF)을 만들었다** — "발표"에 초점을 둔 결과물.

---

## 7. 만든 발표자료 임시 공유 — Cloudflare Drop
