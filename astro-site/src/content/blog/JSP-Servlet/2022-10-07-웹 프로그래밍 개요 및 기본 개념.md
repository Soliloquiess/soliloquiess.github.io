---
title: "[JSP·Servlet] 웹 프로그래밍 기초: WWW·도메인·IP·MVC 패턴"
date: 2022-10-07
category: "JSP·Servlet"
tags: ["JSP·Servlet"]
description: "WWW의 탄생부터 정적/동적 웹, 웹서버·WAS 구분, 도메인·IP 주소 구조, 디자인 패턴, MVC 및 Spring MVC 핵심 개념까지 웹 프로그래밍 기초를 체계적으로 정리한 노트."
permalink: "study/2022/10/07/웹-프로그래밍-개요-및-기본-개념"
---

# 1. 월드와이드웹

## 1) WWW의 탄생
- 영국의 **팀 버너스리**가 ‘**하이퍼텍스트(hypertext)**’ 개념을 고안
...
2. 브라우저가 **DNS(Domain Name System) 서버**를 통해 도메인을 **IP 주소로 변환**  
...
# 6. MVC 디자인 패턴 (MVC Design Pattern)
...
| 구성요소 | 역할 |
|----------|------|
| **DispatcherServlet** | 클라이언트의 요청을 분석하여 가장 적합한 핸들러(Controller method)를 찾음 |
| **HandlerMapping** | 요청에 맞는 핸들러(Controller method)를 반환 |
| **HandlerAdapter** | 해당 핸들러(Controller method)를 실제로 호출 |
| **ViewResolver** | 응답에 적합한 뷰(View)를 찾아 반환 |

> 참고: MVC 패턴에서 Model은 일반적으로 **비즈니스 로직 계층**을 의미하지만,  
> **Spring에서는 Service 계층이 이 역할을 담당**함.

---

## 4) 스프링 MVC 프레임워크를 사용하는 이유
