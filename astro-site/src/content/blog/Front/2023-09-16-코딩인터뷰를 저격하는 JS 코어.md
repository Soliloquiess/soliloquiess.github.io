...
category: "Front"
tags: ["Front"]
description: "JS 코어 JS의 고유 개념 및 타입 (원시타입, 참조타입, 래퍼타입) 원시타입의 특징 원시값을 변수에 할당하면 값이 복사되어 들어감 즉, 원시값이 할당된 변수들은 모두 자기 자신만의 고유한 값을 지니게 됨…"
...
```text
// 두 인스턴스가 프로토타입의 동일한 smell 함수를 공유하므로 true
myFood.smell === myFood2.smell  // true
```
