---
title: "[Java] java 자료구조"
date: 2021-03-20
category: "Java"
tags: ["Java"]
description: "배열·연결 리스트·스택·큐·트리·그래프·해싱 등 주요 자료구조 개념과 직접 구현, 제네릭 프로그래밍, 그리고 컬렉션 프레임워크(List·Set·Map) 활용을 정리한다."
permalink: "study/2021/03/20/java_자료구조"
---

> 작성일: 2021-03-21

# 01. 여러 가지 자료구조에 대해 알아봅시다
...
![배열(Array) 구조](/assets/array.png)

**연결 리스트 (LinkedList)**: 선형으로 자료를 관리한다. 자료가 추가될 때마다 메모리를 할당받고, 자료는 링크로 연결된다. 자료의 물리적 위치와 논리적 위치가 다를 수 있다.

리스트에 자료 추가하기 <br>
![연결 리스트에 자료 추가하기](/assets/listadd.png)

리스트에서 자료 삭제하기 <br>

![연결 리스트에서 자료 삭제하기](/assets/listdelete.png)

**스택 (Stack)**: 가장 나중에 입력된 자료가 가장 먼저 출력되는 자료구조 (Last In First Out) <br>

![스택(Stack) 구조 - LIFO](/assets/stack_3iwsv56kv.png)

**큐 (Queue)**: 가장 먼저 입력된 자료가 가장 먼저 출력되는 자료구조 (First In First Out) <br>

![큐(Queue) 구조 - FIFO](/assets/queue.png)

### 트리 (Tree)

부모 노드와 자식 노드 간의 연결로 이루어진 자료구조다. <br>

**힙 (heap)**: 우선순위 큐(Priority Queue)를 구현한다. 우선순위 큐는 우선순위가 높은 순으로 꺼내므로 힙을 이용해서 구현한다. (최대 힙은 부모 노드가 자식 노드보다 항상 크기 때문이며, 반대인 최소 힙으로도 가능하다.)

- Max heap: 부모 노드가 자식 노드보다 항상 크거나 같은 값을 갖는 경우
- Min heap: 부모 노드가 자식 노드보다 항상 작거나 같은 값을 갖는 경우

heap 정렬에 활용할 수 있다.

**이진 트리 (binary tree)**: 부모 노드에 자식 노드가 두 개 이하인 트리 <br>

![이진 트리(binary tree)](/assets/binary1.png)

**이진 검색 트리 (binary search tree)**

![이진 검색 트리(binary search tree)](/assets/binary3.png)

- 자료(key)의 중복을 허용하지 않는다.
- 왼쪽 자식 노드는 부모 노드보다 작은 값, 오른쪽 자식 노드는 부모 노드보다 큰 값을 가진다.
- 자료 검색에 걸리는 시간이 평균 log(n)이다.
- inorder traversal 탐색을 하면 자료가 정렬되어 출력된다.

예) [23, 10, 28, 15, 7, 22, 56] 순으로 자료를 넣을 때의 BST <br>

나중에 TreeSet이나 TreeMap을 할 때 Comparable 인터페이스를 구현해서 비교하게끔 구현할 것이다.

![BST 구성 예시](/assets/binary2_2n08fto77.png)

- JDK 클래스: TreeSet, TreeMap (Tree로 시작되는 클래스는 정렬을 지원한다.)

### 그래프 (Graph)

정점과 간선들의 유한 집합 G = (V, E)이다. 네비게이션, 구글맵 등이 모두 그래프에 기반해서 구현된다.

- 정점(vertex): 여러 특성을 가지는 객체, 노드(node)
- 간선(edge): 객체들을 연결하는 관계를 나타냄. 링크(link)
- 간선은 방향성이 있는 경우와 없는 경우가 있다. (노드와 노드를 연결하는 것을 edge나 link라고 한다.)
- 그래프를 구현하는 방법: 인접 행렬(adjacency matrix), 인접 리스트(adjacency list)
- 그래프를 탐색하는 방법: BFS(breadth first search), DFS(depth first search)

그래프의 예) <br>

![그래프(Graph) 예시](/assets/graph_tqqi14b2x.png)
...
- `index = h(key)`: 해시 함수가 key에 대한 인덱스를 반환해주며, 해당 인덱스 위치에 자료를 저장하거나 검색
- 해시 함수에 의해 인덱스 연산이 산술적으로 가능하여 O(1)
- 저장되는 메모리 구조를 해시 테이블이라 함
- JDK 클래스: HashMap, Properties

키는 중복되지 않으며, 나머지를 구하는 것이 대표적인 해시 함수다(예: `123 % 100`을 해서 23번째 자리에 저장).

**해시 테이블** <br>

![해시 테이블 구조](/assets/hash_2yk6qjwjl.png)

**체이닝** <br>

![해시 충돌 처리 - 체이닝](/assets/hash2.png)
...
- 키는 유일하여 중복될 수 없다.
- 해시는 들어가는 순서와 상관이 없다.
- 배열과 비슷하게 생겨서 많이 오해하는데, 해시 함수에 의해 위치가 정해지므로 순서와는 무관하다.
...
- 연결 리스트의 i 번째 요소를 찾는 데 걸리는 시간은 요소의 개수에 비례: O(n)
- JDK 클래스: LinkedList
...
- 클래스에서 사용하는 변수의 자료형이 여러 개일 수 있고 그 기능(메서드)은 동일한 경우, 클래스의 자료형을 특정하지 않고 추후 해당 클래스를 사용할 때 지정할 수 있도록 선언한다.
...
단, Object를 사용하는 경우는 형 변환을 해야 한다:
```
...
- 자료형 매개변수 T(type parameter): 이 클래스를 사용하는 시점에 실제 사용할 자료형을 지정한다. static 변수에는 사용할 수 없다.
...
![Material 추상 클래스를 상속한 Powder/Plastic 구조](/assets/material.PNG)
...
- 자료형 매개변수를 메서드의 매개변수나 반환 값으로 가지는 메서드다.
- 자료형 매개변수가 하나 이상인 경우도 있다.
- 제네릭 클래스가 아니어도 내부에 제네릭 메서드를 구현하여 사용할 수 있다.
- 형식: `public <자료형 매개 변수> 반환형 메서드 이름(자료형 매개변수.....) { }`
...
![컬렉션 프레임워크 계층 구조](/assets/collection.png)
...
- Member 클래스를 만들고, 아이디와 이름을 멤버 변수로 선언한다.
...
- 컬렉션 프레임워크에 저장된 요소들을 하나씩 차례로 참조하는 것이다.
- 순서가 있는 List 인터페이스의 경우는 Iterator를 사용하지 않고 `get(i)` 메서드를 활용할 수 있다.
- Set 인터페이스의 경우 `get(i)` 메서드가 제공되지 않으므로 Iterator를 활용하여 객체를 순회한다.

## Iterator 사용하기

- `boolean hasNext()`: 이후에 요소가 더 있는지 체크하는 메서드. 요소가 있다면 true를 반환한다.
...
아이디가 동일한 경우 같은 멤버이므로 중복되지 않도록, Member 클래스의 `equals()`와 `hashCode()` 메서드를 재정의한다.
...
- 내부적으로 이진 검색 트리(binary search tree)로 구현된다.
- 이진 검색 트리에 저장하기 위해 각 객체를 비교해야 한다.
- 비교 대상이 되는 객체에 Comparable이나 Comparator 인터페이스를 구현해야 TreeSet에 추가될 수 있다.
- String, Integer 등 JDK의 많은 클래스들이 이미 Comparable을 구현했다.
...
Member 클래스가 아이디 오름차순으로 정렬되게 하기 위해 Comparable 인터페이스를 구현한다.
...
- Map 인터페이스를 구현한 클래스다.
- 가장 많이 사용되는 Map 인터페이스 기반 클래스다.
- key-value를 쌍으로 관리하는 메서드를 구현한다.
- 검색을 위한 자료구조다.
- key를 이용하여 값을 저장하고, key를 이용하여 값을 꺼내오는 방식 - hash 알고리즘으로 구현된다.
- key가 되는 객체는 중복될 수 없고, 객체의 유일성 비교를 위한 `equals()`와 `hashCode()` 메서드를 구현해야 한다.
...
- Map 인터페이스를 구현한 클래스이고, key에 대한 정렬을 구현할 수 있다.
- key가 되는 클래스에 Comparable이나 Comparator 인터페이스를 구현함으로써, key-value 쌍의 자료를 key값 기준으로 정렬하여 관리할 수 있다.
